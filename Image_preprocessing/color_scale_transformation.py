...
def greyscale(name: int):
    ...
    grey_image.save(f"images/{name}_grey.jpg")


def greyscale_increase_contrast(name: int):
    # Read the grayscale image
    grey_image = cv2.imread(f"images/{name}.jpg", cv2.IMREAD_GRAYSCALE)
    # Apply histogram equalization
    grey_equalized_image = cv2.equalizeHist(grey_image)
    # Save the image
    cv2.imwrite(f"images/{name}_grey_contrast.jpg", grey_equalized_image)
...
def rgb_to_svm(name: int):
    ...
    cv2.imwrite(f"images/{name}_HSV.jpg", img)
